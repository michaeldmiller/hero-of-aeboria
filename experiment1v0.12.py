# first experiment, version 0.12
# changelog: code cleaned a bit for first github commit
    ...
    def update(self):
        ...
        if collision:
            ...
            if len(collision) > 1:
                ...
                # currently buggy and does not handle more than one additional collision
                new_collision = list(collision)
                del new_collision[location_of_lowest]
                for remaining in new_collision:
                    if remaining.rect.x > self.hero.rect.x + (0.5 * self.hero.rect.width):
                        # right
                        self.hero.acceleration.x = 0
                        self.hero.velocity.x = 0
                        self.hero.position.x = remaining.rect.left - (0.5 * self.hero.rect.width)
                        # self.hero.cant_go_left = True
                    elif remaining.rect.x < self.hero.rect.x - (0.5 * self.hero.rect.width):
                        # left
                        self.hero.acceleration.x = 0
                        self.hero.velocity.x = 0
                        self.hero.position.x = remaining.rect.right + (0.5 * self.hero.rect.width)
                        # self.hero.cant_go_right = True
                    elif remaining.rect.y < self.hero.rect.y:
                        # below
                        # bounce back with equivalent or a maximum y velocity
                        if abs(self.hero.velocity.y) > 0:
                            if abs(self.hero.velocity.y) > 1:
